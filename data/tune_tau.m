function [opt_tau1,opt_tau2,opt_tau3,opt_tau4,opt_tau5,...
    ...
    opt_C0,opt_C1,opt_C2,opt_C3,opt_C4,opt_C5,opt_C6,opt_C7,h,count_]= tune_tau(Ctrain,dtrain,Ctest,dtest,C,kernel,p1,s,lamb1,lamb2,K_fold,count_pre)
tauval= -1:0.05:1;
...
acc6_ = zeros(1,length(tauval));acc7_ = zeros(1,length(tauval));count=count_pre;
%%
for jj=1:length(tauval)
        time0=0;time1=0;time2=0;time3=0;
        fprintf('%3.0f steps remaining...\n',length(tauval)*K_fold-count);
        tau= tauval(jj);
        tic
